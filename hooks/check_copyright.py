...
def check_copyright(py_files: list[str]) -> int:
    retv = 0
    with open("LICENCE.md", "r") as txt:
        scooper_licence = txt.read().replace("*", "")
    for py_file in py_files:
        with open(py_file, "r+") as f:
            content = f.read()
            if scooper_licence not in content:
                f.seek(0)
                f.write('"""\n' + scooper_licence + '"""\n' + content)
                print(f"Copyright header added to {py_file}")
                retv = 1
    return retv
...
def main() -> int:
    py_files = sys.argv[1:]
    return check_copyright(py_files)
